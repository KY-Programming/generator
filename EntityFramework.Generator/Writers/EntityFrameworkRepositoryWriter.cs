...
public class EntityFrameworkRepositoryWriter : Codeable
{
    ...
    public virtual void Write(EntityFrameworkWriteConfiguration configuration)
    {
        ...
        {
            EntityTransferObject entity = this.transferObjects.OfType<EntityTransferObject>().FirstOrDefault(x => x.Name == repositoryConfiguration.Entity)
            ...
            ClassTemplate repository = this.files.AddFile(configuration.RelativePath, generatorOptions)
                                           .WithName(Formatter.FormatFile(className, generatorOptions))
                                           .AddNamespace(repositoryConfiguration.Namespace ?? configuration.Namespace)
                                           .AddClass(className)
                                           .FormatName(generatorOptions)
