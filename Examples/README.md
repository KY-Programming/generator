...
Options that are not tied to one language - they shape whatever the writers of a run produce.

| Example | What it shows |
|---|---|
| [Annotation/HeaderAndLint](Common/Annotation/HeaderAndLint) | Dropping the generator version from the `<auto-generated>` header with `[assembly: GenerateNoHeaderVersion]` and replacing the linter comment of a single model - for every language or for one of them - with `[GenerateSuppressLint]` - [AssemblyInfo.cs](Common/Annotation/HeaderAndLint/AssemblyInfo.cs). |
| [Fluent/HeaderAndLint](Common/Fluent/HeaderAndLint) | The same two options as `NoHeaderVersion()` and `SuppressLint(...)` on the write, and limiting the replaced comment to one language of a write that produces several - [GeneratorMain.cs](Common/Fluent/HeaderAndLint.Generator/GeneratorMain.cs). |
