...
export class StatusApiVersionService {
    ...
    public get(httpOptions?: {}): Observable<string> {
        ...
        let url: string = this.serviceUrl + "/api/v1/statusapiversion";
    ...
    public getPreview(httpOptions?: {}): Observable<string> {
        let subject = new Subject<string>();
        httpOptions = { responseType: 'text', ...this.httpOptions, ...httpOptions};
        let url: string = this.serviceUrl + "/api/v2.0-beta/statusapiversion/preview";
        this.http.get<string>(url, httpOptions).subscribe((result) => {
            subject.next(this.fixUndefined(result));
            subject.complete();
        }, (error) => subject.error(error));
