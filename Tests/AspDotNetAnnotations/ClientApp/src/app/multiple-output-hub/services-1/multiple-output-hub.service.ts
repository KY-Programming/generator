...
import { ConnectionStatus } from "../models-1/connection-status";
...
export class MultipleOutputHubService {
    ...
    private readonly connectErrorSubject: Subject<Error> = new Subject<Error>();
    public readonly connectError$: Observable<Error> = this.connectErrorSubject.asObservable();
    ...
    public connect(trial: number = 0): Observable<void> {
        ...
        let startConnection: () => Observable<void> = () => {
            ...
            hubConnection.start().then(() => {
                ...
                const timeout: number = this.timeouts[trial] ??  this.timeouts[this.timeouts.length - 1] ??  5000;
                trial++;
                setTimeout(() => {
                    if (this.isClosed) {
                        return;
